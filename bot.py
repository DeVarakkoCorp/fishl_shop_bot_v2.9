...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("rank57_item:"):
        ...
        await query.edit_message_text(
            f"🛒 *Заказ услуги*\n\n*{category}*\n{item_name}\n\n"
            "Напиши дополнительные детали к заказу. Если дополнительных деталей нет — напиши `НЕТ`.",
            parse_mode="Markdown"
        )
        return
    ...
    if data.startswith("service_item:"):
        ...
        await query.edit_message_text(
            f"🛒 *Заказ услуги*\n\n*{category}*\n{item_name}\n\n"
            "Напиши дополнительные детали к заказу (персонаж, количество, конкретные предметы и т. п.).\n"
            "Если дополнительных деталей нет — напиши `НЕТ`.",
            parse_mode="Markdown"
        )
        return
    ...
    if data.startswith("tier:"):
        ...
        await query.edit_message_text("🆔 *Шаг 1 из 2*\n\nОтправь UID аккаунта Genshin Impact.\n\nНапример: `123456789`", parse_mode="Markdown")
        return


async def receive_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Route client text to exactly one active order step."""
    if context.user_data.get("waiting_for_uid"):
        await receive_uid(update, context)
    elif context.user_data.get("waiting_for_details"):
        await receive_details(update, context)
    elif any(context.user_data.get(k) for k in ("waiting_for_login", "waiting_for_password", "waiting_for_code")):
        await receive_credentials(update, context)


async def receive_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    order = orders.get(user_id)
    if not order or not context.user_data.get("waiting_for_details"):
        return
    order["details"] = update.message.text.strip()
    context.user_data.clear()
    context.user_data["waiting_for_uid"] = True
...
async def receive_uid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not context.user_data.get("waiting_for_uid"):
        return
    uid = update.message.text.strip()
    if not uid.isdigit() or not 5 <= len(uid) <= 12:
        await update.message.reply_text("❌ UID должен состоять только из цифр.\nНапример: `123456789`", parse_mode="Markdown")
        return
    orders.setdefault(user_id, {})["uid"] = uid
    context.user_data["waiting_for_uid"] = False
    context.user_data["waiting_for_server"] = True
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🇪🇺 Европа", callback_data="server:Европа")],
        [InlineKeyboardButton("🇺🇸 Америка", callback_data="server:Америка")],
        [InlineKeyboardButton("🇦🇺 Азия", callback_data="server:Азия")],
        [InlineKeyboardButton("🇨🇳 TW/HK/MO", callback_data="server:TW/HK/MO")],
    ])
    await update.message.reply_text("🌐 *Шаг 2 из 2*\n\nВыбери сервер:", reply_markup=kb, parse_mode="Markdown")


async def server_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not context.user_data.get("waiting_for_server"):
        return
    user_id = update.effective_user.id
    ...
    if not order or "uid" not in order:
        await query.edit_message_text("❌ Данные заказа потеряны.\n\nНажми /start и создай заказ заново.", reply_markup=MAIN_MENU)
        return
    order["server"] = server
    service_name = order.get("service", "Зачистка по регионам")
    warning = (
        "⚠️ *Важно перед передачей данных для входа*\n\n"
        f"Для выполнения услуги «{service_name}» менеджеру могут понадобиться данные для входа в аккаунт. "
        "Никому не передавай эти данные, кроме исполнителя, которому доверяешь. "
        "Заказчик самостоятельно принимает на себя все риски и ответственность, связанные с передачей данных для входа, "
        "доступом к аккаунту и действиями, выполненными в рамках заказа.\n\n"
        "После выполнения заказа рекомендуется изменить пароль.\n\n"
        "Продолжая, ты подтверждаешь, что самостоятельно принимаешь эти риски и ответственность."
    )
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Принимаю риски и ответственность", callback_data="accept_risks")],
        [InlineKeyboardButton("❌ Отменить", callback_data="cancel_order")],
    ])
    await query.edit_message_text(warning, reply_markup=kb, parse_mode="Markdown")


async def accept_risks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = update.effective_user.id
    order = orders.get(user_id)
    if not order or "server" not in order:
        await query.edit_message_text("❌ Данные заказа потеряны. Нажми /start и создай заказ заново.", reply_markup=MAIN_MENU)
        return
    context.user_data["waiting_for_login"] = True
    await query.edit_message_text(
        "🔐 *Данные для входа — шаг 1 из 3*\n\n"
        "Отправь логин аккаунта.\n\n"
        "⚠️ Не отправляй сюда данные от своей почты, банковских аккаунтов или других сервисов — только данные от аккаунта, который используется для заказа.",
        parse_mode="Markdown"
    )
...
async def receive_credentials(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("waiting_for_login"):
        if not text or len(text) > 256:
            await update.message.reply_text("❌ Некорректный логин. Отправь логин ещё раз.")
            return
        order["login"] = text
        context.user_data["waiting_for_login"] = False
        context.user_data["waiting_for_password"] = True
        await update.message.reply_text(
            "🔐 *Данные для входа — шаг 2 из 3*\n\nОтправь пароль аккаунта.",
            parse_mode="Markdown"
        )
        return
    ...
    if context.user_data.get("waiting_for_password"):
        ...
        await update.message.reply_text(
            "🔐 *Данные для входа — шаг 3 из 3*\n\nОтправь код подтверждения входа.\n\nЕсли код сейчас не требуется, напиши `НЕТ`.",
            parse_mode="Markdown"
        )
        return
    ...
    if context.user_data.get("waiting_for_code"):
        if not text or len(text) > 64:
            await update.message.reply_text("❌ Некорректный код. Отправь код ещё раз или напиши `НЕТ`.")
            return
...
async def confirm_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur = conn.execute("INSERT INTO orders (created_at, client_telegram_id, client_name, uid, server, region, tier, price, service, details) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                       (now, user_id, display_name, order["uid"], order.get("server", "Не указан"), order.get("region", "Не требуется"), tier_name, order["price"], order.get("service", "Зачистка по регионам"), order.get("details", "")))
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(server_button, pattern=r"^server:"))
    ...
    app.add_handler(CallbackQueryHandler(accept_risks, pattern=r"^accept_risks$"))
